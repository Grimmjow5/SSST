...
class ReportExtController extends Flight {
    ...
    public function GenerateExtPDF() {
        ...
        $pdf->setPaper('A4','landscape');
           ...
           $this->ValidateRequest($_REQUEST);
            ...
            $pdf->stream("RepExtintor{$hoy}.pdf",array("Attachment"=> false));
    ...
    public function GenerateExtExcel(){
        ...
        $headExcel = ['No','Fecha de Registro','Lugar designado','Acceso obstruido','Señalamiento Obstruido','Instrucciones Legibles','Sellos en condiciones','Rango Operable','Daño Fisico','Accesorios en buen estado','Altura','Peso','Fecha Ultima Recarga','Fecha Proxima Recarga','Subarea','Area'];
        $this->ValidateRequest($_REQUEST);
        $reportE = $this->factory->reportesExt->GetReportsExt($this->modelExtintor);
        ...
        header("Content-Disposition: attachment;filename=ReporteExtGenerado{$hoy}.xlsx");
        ...
        foreach ($datos as $key) {
           array_push   ($newArray,
                                [
                                    ...
                                    $key['altura'],
                                    $key['peso'],
                                    $this->pdf->dateFormat($key['fecha_recarga']),
                                    $this->pdf->dateformat($key['fecha_prox_recarga']),
                                    $key['id_sub'],
                                    $key['id_area']
                                ]);
        }
