...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertCatExtintor` (IN `p_num_inventario` VARCHAR(50), IN `p_fecha_registro` DATETIME, IN `p_num_extintor` VARCHAR(50), IN `p_estatus` BIT, IN `p_idarea` INT)   BEGIN
    INSERT INTO cat_extintores (num_inventario, fecha_registro, num_extintor, estatus,id_area)
    VALUES (p_num_inventario, p_fecha_registro, p_num_extintor, p_estatus,p_idarea);
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateCatExtintor` (IN `p_id_extintor` INT, IN `p_num_inventario` VARCHAR(50), IN `p_num_extintor` VARCHAR(50), IN `p_estatus` BIT, IN `p_idarea` INT)   BEGIN
    UPDATE cat_extintores
    SET num_inventario = p_num_inventario, num_extintor = p_num_extintor, estatus = p_estatus , id_area=p_idarea
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateUsuarios` (IN `p_user` VARCHAR(40), IN `p_user_name` VARCHAR(60), IN `p_last_name` VARCHAR(40), IN `p_pass` VARCHAR(300), IN `p_id_rol` INT, IN `p_status` BIT)   BEGIN
    UPDATE reg_extintores
    SET 
        id_area = p_id_area,
        user_name = p_lugar_designado,
...
--
ALTER TABLE `cat_extintores`
  ADD CONSTRAINT `CatArea` FOREIGN KEY (`id_area`) REFERENCES `cat_areas` (`id_area`);
